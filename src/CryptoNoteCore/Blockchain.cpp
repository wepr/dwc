...
Crypto::Hash Blockchain::getPrevBlockHash() 
	...
	if (height<=1) 
		...
		return getBlockIdByHeight(height-1);
	}
//////
...
difficulty_type Blockchain::getDifficultyForNextBlock() {
  ...
  return m_currency.nextDifficulty(timestamps, commulative_difficulties, m_blocks.size());
}
...
//////////////////////
	if(height<2)
		...
		z_lock = height + m_currency.minedMoneyUnlockWindow();

	if (!(b.baseTransaction.unlockTime == z_lock)) 
	{
		logger(ERROR, BRIGHT_RED)
...
//$$$$	
  ...
  if (b.timestamp > get_adjusted_time() + ts) {
    logger(INFO, BRIGHT_WHITE) <<
      "Timestamp of block with id: " << get_block_hash(b) << ", " << b.timestamp << ", bigger than adjusted time +" << ts <<" s.";
    return false;
  }
...
bool Blockchain::pushBlock(const Block& blockData, const std::vector<Transaction>& transactions, block_verification_context& bvc) {
  ...
  if (!validate_miner_transaction(blockData, static_cast<uint32_t>(m_blocks.size()), cumulative_block_size, already_generated_coins, fee_summary, reward, emissionChange, getPrevBlockHash())) {
