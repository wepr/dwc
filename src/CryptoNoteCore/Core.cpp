...
//getBlockIdByHeight(getCurrentBlockchainHeight)
//uint32_t che;
//che = 
//m_blockchain.getBlockIdByHeight(0)
//getPrevBlockHash
  /*
  ...
  bool r = m_currency.constructMinerTx(height, median_size, already_generated_coins, txs_size, fee, adr, b.baseTransaction, ex_nonce, 11, m_blockchain.getPrevBlockHash());
  if (!r) { 
  ...
  for (size_t try_count = 0; try_count != 10; ++try_count) {
    r = m_currency.constructMinerTx(height, median_size, already_generated_coins, cumulative_size, fee, adr, b.baseTransaction, ex_nonce, 11, m_blockchain.getPrevBlockHash());
...
bool core::getBlockReward(size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee, uint32_t height,
  ...
  return m_currency.getBlockReward(medianSize, currentBlockSize, alreadyGeneratedCoins, fee, height, reward, emissionChange, last_BlockHash);
}
